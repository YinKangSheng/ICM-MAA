...
class AAFG(Attack):
    ...
    def forward(self, images, targets, metas, pil_bound_maxs, pil_bound_mins):
        r"""
        Overridden.
        """
        os.environ["CUDA_VISIBLE_DEVICES"] = '7'
        images = images.clone().detach().to(self.device)
        pil_bound_maxs = pil_bound_maxs.clone().detach().to(self.device)
        pil_bound_mins = pil_bound_mins.clone().detach().to(self.device)
        ...
        for _ in range(self.steps):
            ...
            delta = torch.clamp(adv_images - images, min=-pil_bound_mins, max=pil_bound_maxs)
